...
/* Helpers */
import { getCurrency, getLanguage } from "../../../helpers/formatHelper";
import texts from "../../../helpers/Text-En.json";

const DetailSectionMovie = (props) => {
  return (
      ...
      <div className="flex flex-col gap-6">
        <div className="flex flex-row align-middle gap-2">
          <strong className="text-nowrap">
            {texts.Details_OriginalLanguage}
          </strong>
          <FaCircle className="w-1 h-1 self-center" />
          <span className="text-primaryOrange">
            {getLanguage(props.originalLanguage)}
          </span>
        </div>
        <div className="flex flex-row align-middle gap-2">
          <strong className="text-nowrap">{texts.Details_OriginalTitle}</strong>
          <FaCircle className="w-1 h-1 self-center" />
          <span className="text-primaryOrange">{props.originalTitle}</span>
        </div>
        {props.budget > 0 && (
          <div className="flex flex-row align-middle gap-2">
            <strong className="text-nowrap">{texts.Global_Budget}</strong>
            <FaCircle className="w-1 h-1 self-center" />
            <span className="text-primaryOrange">
              {getCurrency(props.budget)}
            </span>
          </div>
        )}
...
export default DetailSectionMovie;
