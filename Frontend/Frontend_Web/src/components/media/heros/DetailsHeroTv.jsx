...
const DetailsHeroTv = ({
  ...
  async function handleWatchlist() {
    if (userLoggedIn) {
      if (onUserWatchlist) {
        const result = await removeFromUserWatchListAsync(id, texts.Global_tv);
        result ? setOnUserWatchlist(false) : setOnUserWatchlist(true);
      } else {
        const dataToAdd = {
          mediaId: id,
          mediaType: texts.Global_tv,
          name: name,
        ...
        <div className="relative w-full h-[400px] bg-overLayLightDark lg:h-[700px]">
          ...
          <div className="default-container flex flex-row text-primaryLightGrey p-4 justify-center lg:py-8 xl:px-0">
            ...
            <div className="flex flex-col w-full gap-3 py-4 pl-4 lg:pt-4 lg:pl-0">
              <div>
                <div className="text-headingLg">{name}</div>
                <div className="text-textMd">
                  {texts.Global_CreatedBy}{" "}
              ...
              <div className="flex flex-row gap-4">
                <div className="text-headingSm">{firstAirDate}</div>
                {episodeRuntime && (
                  <div className="text-headingSm">
                    {episodeRuntime} {texts.Global_min}
                  </div>
                )}
              </div>
